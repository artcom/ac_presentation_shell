...
- (void)play:(NSString *)file withDelegate:(id<KeynotePlaybackDelegate>)delegate {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (presentation && [self presentationShouldStillRun:ticket]) {
            ...
            if ([self keynoteIsPlayingFullscreen:[presentation name]]) {
                dispatch_async(dispatch_get_main_queue(), ^{
...
- (BOOL)keynoteIsPlayingFullscreen:(NSString *)name
{
    for (KeynoteWindow *window in self.application.windows) {
        /**
           09.11.2023:
           Keynote keeps closed windows around. When changing playback settings (fullscreen / in window) ACShell gets confused.
           Solution: we will look for windows which are visible and have the document name.
           Then we will check if the window has no controls.
         */
        if (window.visible && [window.name isEqualToString:name]) {
            if (!window.closeable && !window.resizable && !window.zoomable) {
                return YES;
            }
        }
    }
